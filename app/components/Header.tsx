...
export default function Header() {
  ...
  // NEW: Track if app is installed (PWA)
  const [isStandalone, setIsStandalone] = useState(false);
  const [isMounted, setIsMounted] = useState(false);

  // NEW: Check if PWA is installed (only runs on client)
  useEffect(() => {
    setIsMounted(true);
  ...
  const navItems = [
    { href: "/impact", label: "Our Impact", icon: SparklesIcon },
      ...
      <header className="fixed inset-x-0 top-0 z-50 bg-white/95 backdrop-blur-xl border-b border-gray-100 shadow-sm">
        <nav className="max-w-7xl mx-auto px-4 sm:px-6 h-20 flex items-center justify-between">
          ...
          <div className="hidden lg:flex items-center gap-2">
            ...
            {/* Cart - Desktop (only for buyers or guests) */}
            ...
            {/* PWA Install Button - FIXED: Only render after client mount */}
            <InstallButton />
          </div>
